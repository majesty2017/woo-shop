...
export class BackendInterceptor implements HttpInterceptor {
  ...
  //  This will skip the interceptor if  SkipInterceptor header if present
    if (request.headers.has(SkipInterceptor)) {
      const headers = request.headers.delete(SkipInterceptor);
      return next.handle(request.clone({headers}));
    }

  //  This will also skip the interceptor if  Write-Object header if present
  ...
  //  if both header are not present, process normally
